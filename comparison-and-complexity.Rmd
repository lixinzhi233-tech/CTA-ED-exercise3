...
```{r}
# 1. cacluate distance (take education as example)
dist_euclidean <- textstat_dist(dfmat, 
                                margin = "documents", 
                                method = "euclidean")

# 2. turn to matrix and then take Theresa May
dist_matrix <- as.matrix(dist_euclidean)
may_dist <- as.data.frame(dist_matrix[23, c(1:22, 24)])

# 3. prepare for visualation
colnames(may_dist) <- "dist_score"
may_dist <- tibble::rownames_to_column(may_dist, "username")

# 4. visualaiton
ggplot(may_dist, aes(x = reorder(username, dist_score), y = dist_score)) +
  geom_point() +
  coord_flip() +
  labs(title = "Euclidean Distance to Theresa May", y = "Distance Score") +
  theme_minimal()
...
```{r}
# caculate Gunning Fog, Coleman-Liau, SMOG
comp_multi <- textstat_readability(speeches$text,
                measure = c("Flesch.Kincaid", "FOG", "Coleman.Liau.grade", "SMOG"))
